...
name: {{agent_slug}}
description: >
  One-line trigger description. The orchestrator reads this to decide
...
## Operating Procedure
1. **Lookup first** — run `rg -li "<key term>" wiki/` before any external search. If the wiki already has good coverage, summarize it and ask if external research is still needed.
